...
export default function ReaderScreen(){
 ...
 const {getDocument,touchDocument,updateProgress,toggleFavorite,settings}=useApp(); const {t}=useTranslation();
 ...
 return <View style={styles.root}>
   ...
   <View style={styles.toolbar}><ReaderToggle icon="rotate" active={horizontal} label={horizontal?t('reader.horizontal'):t('reader.vertical')} onPress={()=>setHorizontal(v=>!v)}/><ReaderToggle icon="snap" active={paging} label={paging?t('reader.singlePage'):t('reader.flow')} onPress={()=>setPaging(v=>!v)}/><View style={styles.pageChip}><Text style={styles.pageStrong}>{page}</Text><Text style={styles.pageMuted}> / {count||'—'}</Text></View><ReaderToggle icon="moon" active={inverted} label={t('reader.night')} onPress={()=>setInverted(v=>!v)}/></View>
   <AdBanner />
   <Modal visible={passwordOpen} transparent animationType="fade" onRequestClose={()=>setPasswordOpen(false)}><View style={styles.modalBg}><View style={styles.modalCard}><Text style={styles.modalTitle}>{t('reader.passwordTitle')}</Text><Text style={styles.modalText}>{t('reader.passwordText')}</Text><TextInput secureTextEntry value={passwordDraft} onChangeText={setPasswordDraft} placeholder={t('reader.passwordPlaceholder')} placeholderTextColor="#64748B" style={styles.passwordInput}/><Pressable onPress={()=>{setPassword(passwordDraft);setPasswordOpen(false);setError('')}} style={styles.unlock}><Text style={styles.unlockText}>{t('reader.passwordUnlock')}</Text></Pressable></View></View></Modal>
 </View>;
}
