...
import { useAdsRefresh, useAdsStatus } from '@/context/AdsContext';
...
export default function SettingsScreen(){
 ...
 const adsStatus=useAdsStatus(); const {paused,remainingMinutes}=useAdPause(); const [rewardBusy,setRewardBusy]=useState(false);
 // No feature is ever locked behind this. The viewer trades attention for a
 // completely ad free window, which is the only "remove ads" option an app
 // without in app purchases can honestly offer.
 async function watchForAdFreeTime(){
  if(rewardBusy||paused)return;
  setRewardBusy(true);
  try{
...
function SettingRow({icon,title,desc,value,onValueChange}:{icon:React.ComponentProps<typeof AppIcon>['name'];title:string;desc:string;value:boolean;onValueChange:(next:boolean)=>void}){return <View style={styles.settingRow}><View style={styles.actionIcon}><AppIcon name={icon} size={19} color={palette.pdfRedSoft}/></View><View style={{flex:1}}><Text style={styles.actionTitle}>{title}</Text><Text style={styles.actionDesc}>{desc}</Text></View><Switch value={value} onValueChange={onValueChange} accessibilityRole="switch" accessibilityLabel={title} accessibilityHint={desc} accessibilityState={{checked:value}} trackColor={{false:palette.metalSoft,true:palette.pdfRedDeep}} thumbColor="#F8FAFC"/></View>}
const styles=StyleSheet.create({safe:{flex:1,backgroundColor:palette.ink},content:{padding:18,gap:22,paddingBottom:32},kicker:{color:palette.pdfRedSoft,fontSize:9,fontWeight:'900',letterSpacing:1.8},title:{color:palette.white,fontSize:25,fontWeight:'900',marginTop:4},sub:{color:palette.muted,fontSize:12,lineHeight:18,marginTop:6},sectionTitle:{color:palette.steel,fontSize:9,fontWeight:'900',letterSpacing:1.5,marginStart:3},sectionBox:{borderRadius:18,backgroundColor:palette.card,borderWidth:1,borderColor:palette.line,overflow:'hidden'},settingRow:{minHeight:72,flexDirection:'row',alignItems:'center',paddingHorizontal:13,gap:11,borderBottomWidth:StyleSheet.hairlineWidth,borderBottomColor:palette.line},actionRow:{minHeight:76,flexDirection:'row',alignItems:'center',paddingHorizontal:13,gap:11},actionDivider:{borderBottomWidth:StyleSheet.hairlineWidth,borderBottomColor:palette.line},actionRowMuted:{opacity:0.72},actionIcon:{width:40,height:40,borderRadius:12,backgroundColor:palette.redGlow,alignItems:'center',justifyContent:'center'},actionTitle:{color:palette.white,fontSize:13,fontWeight:'800'},actionDesc:{color:palette.muted,fontSize:10.5,lineHeight:15,marginTop:4},about:{flexDirection:'row',justifyContent:'center',alignItems:'center',gap:7},aboutText:{color:'#666D77',fontSize:10.5}});
