...
```
CI (`.github/workflows/expo-android.yml`) runs the same checks, then `expo prebuild` + `gradlew assembleDebug`
and publishes the APK as an artifact. **The APK is built for free on GitHub Actions — do not spend Emergent
